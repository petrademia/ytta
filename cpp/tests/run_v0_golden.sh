...
for MODE in sync queued; do
  OUT="$(mktemp)"
  "${BIN}" --mode="${MODE}" --fixture "${FIXTURE}" --out "${OUT}" >/dev/null 2>/dev/null
  if ! cmp -s "${OUT}" "${GOLDEN}"; then
    echo "golden drift (${MODE}): output differs from ${GOLDEN}" >&2
    diff -u "${GOLDEN}" "${OUT}" >&2 || true
    rm -f "${OUT}"
    exit 1
  fi
  rm -f "${OUT}"
  echo "v0 golden ok (${MODE})"
done
